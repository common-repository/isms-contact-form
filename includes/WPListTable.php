...
<p class="search-box">
    ...
    public function current_action() {
       if ( isset( $_REQUEST['filter_action'] ) && ! empty( $_REQUEST['filter_action'] ) ) {
        ...
        if ( isset( $_REQUEST['action'] ) && -1 != $_REQUEST['action'] ) {
            return  filter_var($_REQUEST['action'], FILTER_SANITIZE_STRING);
        }

        if ( isset( $_REQUEST['action2'] ) && -1 != $_REQUEST['action2'] ) {
           return  filter_var($_REQUEST['action2'], FILTER_SANITIZE_STRING);
        }
    ...
    protected function months_dropdown( $post_type ) {
        ...
        if ( ! isset( $_GET['post_status'] ) || 'trash' !== $_GET['post_status'] ) {
            ...
            $extra_checks = $wpdb->prepare( ' AND post_status = %s', filter_var($_GET['post_status'], FILTER_SANITIZE_STRING) );
        }
    ...
    public function print_column_headers( $with_id = true ) {
        ...
        if ( isset( $_GET['orderby'] ) ) {
            $current_orderby = filter_var($_GET['orderby'], FILTER_SANITIZE_STRING);
        } else {
            $current_orderby = '';
        }
